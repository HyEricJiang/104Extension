...
const EXTENSION_TITLE = "104 招募工作台";
...
function setIdle(title = EXTENSION_TITLE) {
...
function setOkThenReset(title = "完成") {
  ...
  setTimeout(() => {
    if (!runtimeState.running) setIdle();
...
async function completeRun(finalTabId, message, doneType = "DONE") {
  ...
  const completion = globalThis.RecruitingClipboardPolicy.resolveCompletion(
    message,
    clipboardResult
  );
  const finalMessage = completion.message;
  ...
  await sendToPopup({
    ...
    clipboardState: completion.clipboardState,
    text
  });
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (!supportedTypes.has(msg?.type)) return false;
...
/**
 * 只公開工作流協調器需要的最小介面，避免兩個功能模組彼此耦合。
 */
globalThis.RecruitingCollector = Object.freeze({
  startCollection,
  getState: async () => ({ ...runtimeState, rows: await getRows() }),
  pause: async () => updateRuntimeState({ stopRequested: true })
});
})();
