...
function badgeIdle(title = EXTENSION_TITLE) {
...
function badgeOkThenReset(title = "完成") {
  ...
  setTimeout(() => {
    if (!pdfBadgeState.running) badgeIdle();
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  const supportedCommands = new Set([
    "getStatus",
    "applySettings",
    "getCurrentTabGroupInfo",
    "stopBatch",
    "downloadCurrent",
    "downloadRight",
    "initBadgePDF"
  ]);
  if (!supportedCommands.has(msg?.cmd)) return false;
