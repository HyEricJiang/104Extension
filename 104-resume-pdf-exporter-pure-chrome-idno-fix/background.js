...
function parseResumeInfoFromUrl(url) {
  try {
    const u = new URL(url);
    const pageSource = (u.searchParams.get("pageSource") || "").toLowerCase();
    const ec = u.searchParams.get("ec") || "";

    // A) 搜尋履歷：
    // 來源可能是：
    //   1. /search/SearchResumeMaster?idno=...
    //   2. /ResumeTools/resumePreview?pageSource=search&searchEngineIdNos=...
    // 依照最新需求，搜尋情境只要有 idno，就直接組成 pageSource=search 的預覽頁，
    // 不再依賴 rc / ec 等其他參數。
    const idno =
      u.searchParams.get("idno") ||
      u.searchParams.get("searchEngineIdNos") ||
      "";

    // B) 文件履歷：
    // 來源可能是：
    //   1. /document/master?sn=...
    //   2. /ResumeTools/resumePreview?pageSource=document&snapshotIds=...
    const snapshotId = u.searchParams.get("sn") || u.searchParams.get("snapshotIds") || "";

    // 若已明確標示為 search，優先走 search 模式，避免 preview URL 同時帶空的 snapshotIds 時誤判。
