# 104 履歷篩選與排序 v4.4.0

此資料夾提供鴻揚科技 104 VIP 人才搜尋頁使用的 Tampermonkey 前端腳本。

前端負責讀取履歷卡片、取得履歷詳情、操作掃描介面及呈現結果。職缺規則、權重、核心資格判斷與排序邏輯由公司限定的 Google Apps Script 後端執行，不包含在這個 Git 專案中。

## 安裝

1. 開啟 [`104-hide-resume-cards.user.js`](./104-hide-resume-cards.user.js)。
2. 複製完整內容到 Tampermonkey，取代舊版腳本並儲存。
3. 以鴻揚科技 Google Workspace 帳號登入。
4. 重新整理 104 VIP 人才搜尋頁。
5. 從右下角面板選擇職缺，再按「掃描並依分數排序」。

腳本適用網址：

```text
https://vip.104.com.tw/search/searchResult*
```

## v4 功能

- 支援 Java、C#/.NET、系統分析、QA／軟體測試及 PM 職缺。
- 依核心能力、期待職稱、工作經歷、薪資、穩定性與職缺專屬條件排序。
- QA 會區分純軟體測試、硬體／韌體測試及開發轉測試傾向。
- QA 硬體／軟韌體製造公司背景會扣分並轉人工覆核；明確 BIOS、GMS、硬體韌體驗證，或近期已轉 Android、iOS、BIOS、Golang 後端且期待方向不是 QA 時排除。
- QA 十年以上經歷會標示 OverQualified 人工覆核；Python／自動化測試標的不清楚時仍維持人工覆核。
- TOEIC、英文寫作、海外留學與外商經驗可依 QA 條件加分。
- v4.3.0 採基礎 60 分的可解釋絕對分數；正式資訊本科與數理／資訊相近科系可獲得加分，名校及大型公司背景不再自動扣分。
- 薪資先看期望值，空白或面議才看最近歷史薪資；月薪明確高於 70,000 或年薪達 1,000,000 時排除，動態行情超標或資料不明時轉人工覆核。
- 職缺相關年資會排除實習與兼職、逐月去除重疊；近期短任期命中門檻時排除，明確合約自然結束者轉人工覆核。
- 近期職涯偏移改為人工覆核；Google Sheet 明確列出的博弈公司命中時硬性排除。
- PM 必須能確認至少三年實際 PM 職稱經驗。
- 每次只掃描目前搜尋結果頁面，不會自動翻頁。
- 104 原生已讀、腳本曾開啟或近三個月已主動聯繫的履歷會跳過；備註不再造成永久跳過。
- 結果可展開查看此次實際套用的唯讀加扣分明細。
- 所有結果固定依總分由高到低排列，分數相同時才使用後端排序層級。
- 推薦、人工覆核與全部結果在每次顯示、切換及分頁前都會重新確認排序。
- 掃描進度分為收集履歷、詳情補強、後端評分、排序呈現四階段，顯示各階段完成筆數與總完成百分比。
- 頁面載入時會即時隱藏已讀或近三個月已發出邀約的人選；掃描後會把後端判斷 Tag 標示在 104 原始履歷卡片上。
- Tag 固定放在 96px 照片欄下方並垂直排列，依正向、企業背景、提醒、風險及一般資訊使用不同語意色彩。
- 「本頁全選」可在同一按鈕切換為「取消本頁全選」。
- 「開啟未讀高分履歷」每次依序開啟 15 筆；第一次為第 1–15 名，第二次接續第 16–30 名，已開啟者會標示「已讀」。

## 權限與資料流

- 後端只允許「鴻揚科技有限公司中的所有使用者」存取。
- 前端不下載完整規則，也不提供修改權重的控制項。
- 姓名及履歷連結只留在目前 104 頁面；送至後端的是判斷所需的履歷內容與履歷代碼。
- 後端回傳總分、排序層級、狀態、顯示標籤與該次加扣分明細。

使用者可在瀏覽器回應中看到個別候選人的明細，因此仍可能推測部分權重；Apps Script 專案與規則試算表的編輯權限應限於維護者。

## 維護

正式腳本檔名維持 `104-hide-resume-cards.user.js`，讓既有安裝流程可以直接取代舊版。若後端部署網址變更，請同步更新腳本設定區的 `RULES_API_URL`。

更新後請確認：

- 腳本版本顯示 `4.4.0`。
- 公司帳號可連線，匿名或外部帳號無法使用。
- 五種職缺皆能完成掃描。
- 加扣分明細可展開，且沒有可修改權重的欄位。
