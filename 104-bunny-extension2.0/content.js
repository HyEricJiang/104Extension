(() => {
  ...
  function normalizePhoneValue(value) {
    ...
    /**
     * 核心修正：欄位裡可能同時塞了兩支電話（例如手機 + 市話），
     * 中間可能只用空白分隔，沒有逗號/斜線這種明顯字元可以判斷邊界。
     * 舊版用單一 regex 比對整段文字，遇到「只用空白分隔」的情況，
     * 空白也會被吃進同一個 match，導致兩支號碼被黏成一串。
     *
     * 做法：先用常見分隔符號（逗號、頓號、斜線、換行、空白...）把整段文字切開，
     * 找出第一個「數字數夠多、像是完整電話號碼」的片段，只保留這一段。
     * 如果切完後沒有任何片段單獨湊滿電話應有的數字量，代表原本就是同一支號碼
     * 用空白做格式化（例如 "0912 345 678"），這時 fallback 用整段文字處理，不會誤刪。
     */
    ...
    const firstFullSegment = segments.find((seg) => (seg.match(/\d/g) || []).length >= 7);
    const candidate = firstFullSegment || cleaned;

    const matched = candidate.match(/\+?[\d][\d\s\-()]{6,}/);
    cleaned = matched ? matched[0].trim() : candidate;
  ...
  /**
   * 有些履歷會填兩組電話（例如手機 + 市話），
   * 每一組號碼通常各自包在一個 class="font-weight-bold" 的元素裡（<a> 或 <div> 都有可能）。
   * 這裡直接在「電話所在那一列」裡找出第一個 .font-weight-bold 元素，
   * 就能確保無論頁面上有幾組電話，永遠只拿第一組。
   */
  /**
   * 修正重點：
   * 姓名欄位也是用 .font-weight-bold（見 readName()），
   * 所以搜尋「電話那一列」時，範圍不能抓到同時包含姓名/Email/電話的外層大容器，
   * 否則第一個抓到的 .font-weight-bold 會是姓名，不是電話。
   *
   * 做法：
   * 1. 優先用跟原本 SEL.phone 一樣的結構路徑，直接鎖定第 3 個欄位（電話）的容器。
   * 2. 若結構有變，才 fallback 用 label 文字比對，並在多個候選容器中，
   *    挑「文字內容最短」的那個，代表它最貼近電話欄位本身，而不是外層大容器。
   */
  function getPhoneFieldContainer() {
    const root = getVisibleContactModalRoot();
    if (!root) return null;

    const structured =
      root.querySelector('.modal-body > div > div > div:nth-child(3) > div.col-10') ||
      root.querySelector('.modal-body div > div:nth-child(3) > div.col-10');
    if (structured) return structured;

    const body = root.querySelector('.modal-body') || root;
    const phoneLabels = ['聯絡電話', '手機', '住家電話', '電話'];

    const candidates = Array.from(body.querySelectorAll('div')).filter((el) => {
      const t = textOf(el);
      return t && phoneLabels.some((label) => t.includes(label));
    });
  ...
  function getFirstPhoneEntryElement() {
    const container = getPhoneFieldContainer();
    if (!container) return null;

    const items = Array.from(container.querySelectorAll('.font-weight-bold')).filter((el) => textOf(el));
    if (items.length) return items[0]; // 只取第一組

    if (container.classList?.contains('font-weight-bold') && textOf(container)) return container;
  ...
  function readPhone() {
    // 優先：電話欄位裡第一個 .font-weight-bold 元素，確保多組電話只取第一組
    const firstEntry = getFirstPhoneEntryElement();
    if (firstEntry) {
      const hrefEntry = attrOf(firstEntry, 'href').replace(/^tel:/i, '').trim();
      if (hrefEntry) return normalizePhoneValue(hrefEntry);

      const txtEntry = textOf(firstEntry);
      if (txtEntry) return normalizePhoneValue(txtEntry);
    }
